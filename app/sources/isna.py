...
logger = setup_logging(source="isna")

# RSS feed URL
ISNA_RSS_URL = "https://www.isna.ir/rss"
...
class ISNAWorker(BaseWorker):
    """Worker for ISNA RSS feed."""

    def __init__(self):
        """Initialize ISNA worker."""
        super().__init__("isna")
        self.rss_url = ISNA_RSS_URL
    ...
    async def _extract_article_content(self, url: str) -> Optional[dict]:
        ...
        try:
            ...
            # Extract article body - prefer <article> tag, then fallback selectors
            body_html = ""
            article_selectors = [
                "article",
                ".item-text",
            ...
            if article_tag:
                ...
                for tag in article_tag.find_all(class_=lambda x: x and any(skip in x.lower() for skip in ["ad", "advertisement", "social", "share"])):
                    tag.decompose()
                # Remove meta info blocks (e.g., date, reporter) with class "meta-news"
                for tag in article_tag.find_all(class_=lambda x: x and "meta-news" in x.lower()):
                    tag.decompose()
                # Remove content after "انتهای پیام"
                end_marker = article_tag.find(string=lambda text: text and "انتهای پیام" in text)
                if end_marker:
                    marker_parent = end_marker.parent
                    # Remove everything after the marker (document order)
            ...
            # Extract main image - prefer og:image, then article header image
            image_url = ""
            og_image = soup.find("meta", attrs={"property": "og:image"})
            if og_image and og_image.get("content"):
                image_url = og_image["content"]
            else:
                # Fallback: article header image
                header_image = soup.find("img", class_=re.compile("article|header|main", re.I))
                if header_image:
    ...
    async def _save_article(self, rss_item: dict, article_content: dict, s3_image_url: str) -> None:
        ...
        async with AsyncSessionLocal() as db:
            try:
                ...
                # Normalize category
                normalized_category, preserved_raw_category = normalize_category("isna", raw_category)
    ...
    async def fetch_news(self) -> None:
        """Fetch and process news from ISNA RSS feed."""
        self.logger.info("Starting ISNA fetch cycle")
